...
const MyGeneralInfo = () => {
  ...
  // Sunucudan gelen özgeçmiş bilgisi
  ...
  // Yeni seçilen fotoğrafın URI'si
  const [photoURI, setPhotoURI] = useState(null);

  // Bileşen mount olduğunda myResumeData'yı çekiyoruz
  useEffect(() => {
    fetchMyResumeData();
  }, []);

  const fetchMyResumeData = async () => {
    try {
      setLoadingResume(true);
      // /api/Resume/myResumeData endpoint'i
      const result = await GetRealApi("Resume/myResumeData", navigation);
      if (result) {
        setResumeData(result);
      } else {
        console.warn("API'den boş veya hatalı sonuç döndü.");
      }
    } catch (error) {
  ...
  // resumeData güncellendiğinde formu dolduruyoruz
  useEffect(() => {
    if (resumeData) {
      setForm({
      ...
      // Sunucuda kayıtlı fotoğraf varsa, onu gösteriyoruz (base64).
      setPhotoURI(null);
    }
  }, [resumeData]);
  ...
  const handleSubmit = async () => {
    try {
      if (!resumeData?.id) {
        alert("Geçersiz özgeçmiş ID'si.");
        return;
      }
      setLoadingSubmit(true);

      // Eğer yeni bir fotoğraf seçtiysek base64'e dönüştür, aksi halde var olanı kullan
      ...
      const updated = await PutRealApi("Resume/myResumeData", dataToSend, navigation);
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {/* Seçilen yeni fotoğrafı göster, yoksa sunucudaki fotoğrafı göster */}
        {photoURI ? (
